...
- The project is pre-1.0 (v0.0.x) with no backward-compatibility guarantee across versions. Change interfaces freely when it improves the design; document only the current behavior, and do not add migration notes or upgrade instructions for older versions.
