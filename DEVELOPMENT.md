...
- **Timeouts are hard bounds, and separate ones.** The host imposes `ConnectTimeout` on the handshake and `ListTimeout` on every listing. They exist for different reasons — a handshake can block in a write that cancellation cannot interrupt, while a listing simply has no caller deadline when a server notification triggers it — and they are independent budgets: a slow handshake must not eat the time allowed for the listing that follows, so bringing a server up can take up to the sum. See [What the SDK does that the code works around](#what-the-sdk-does-that-the-code-works-around).
...
2. **The handshake is unbounded only while its write blocks.** `ioConn.Write` checks the context before writing and then performs a write that cannot be interrupted, so `Client.Connect` ignores cancellation exactly as long as that write is stuck — which is until the peer reads. On an in-memory transport (`net.Pipe`, no buffer) a peer that never appears blocks it forever, measured at three leaked goroutines per attempt; over a stdio pipe the request lands in the OS buffer and the context does bound the wait, because what follows is an ordinary response wait (see note 4). Closing the connection is what interrupts the stuck write, which is why `mcphost` connects through a transport that retains the connection so it can be closed on timeout. That covers both cases, but the case it is *needed* for is the blocked write, not waiting for an answer.
...
4. **A pending response does honour its context.** `AsyncCall.Await` selects on `ctx.Done()`, so a `tools/list` against a server that accepted the request and went quiet returns when its context expires — nothing needs to close the connection for it. The session still imposes `ListTimeout` on every listing, for a different reason: a refresh triggered by a server notification arrives with no caller and therefore no deadline of its own, so without one it would wait forever. Do not reach for the handshake's connection-closing wrapper here; there is nothing for it to fix.
